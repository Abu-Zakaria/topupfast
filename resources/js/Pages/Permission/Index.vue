          ...
          <div class="card-content">
            <div class="card-body">
              ...
              <div class="table-container">
                <div class="flex">
                  <input type="text" v-model="query" @keyup="search" class="form-control" placeholder="Search....">
                </div>
                ...
                <table id="data-table" class="table table-bordered display responsive nowrap mb-0" style="width: 100%">
                  ...
                  <tbody v-if="permissions.data.length > 0">
                    <tr v-for="(permission, index) in permissions.data" :key="permission.id">
                      ...
                      <td>{{ permission.default_date_time }}</td>
                      <td v-html="$options.filters.status(permission.status)"></td>
                      <td class="text-center">
                        <a @click.prevent="setData(permission)" href="" class="text-info" role="button"><i class="feather icon-edit"></i></a>
                        <a @click.prevent="remove(permission)" href="" class="text-warning" role="button"><i class="feather icon-trash"></i></a>
                      </td>
                    </tr>
                  </tbody>
                </table>
...
<script>
    ...
    export default {
        ...
        methods: {
          ...
          remove: async function (permission) {
            if (await this.$confirm()) {
              this.$inertia.delete(this.route('permissions.destroy', permission.id));
              this.$toast(`${permission.name } deleted successfully`);
            }
          },
          search: function () {
            $("#default").modal('hide');
