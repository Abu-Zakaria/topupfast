<template>
  <layout name="InvoiceType">
    <!-- invoice_type list start -->
        ...
        <div class="card">
          <div class="card-content">
            <div class="card-body">
              ...
              <div class="table-responsive" v-if="invoice_types.data.length > 0">
                <table id="data-table" class="table table-bordered display responsive nowrap mb-0" style="width: 100%">
                  ...
                  <tbody>
                    <tr v-for="(invoice_type, index) in invoice_types.data" :key="invoice_type.id">
                      <th>{{ index + 1 }}</th>
                      <th>{{ invoice_type.name }}</th>
                      <td>{{ invoice_type.default_date_time }}</td>
                      <td v-html="$options.filters.status(invoice_type.status)"></td>
                      <td class="text-center">
                        <a @click.prevent="setData(invoice_type)" href="" class="text-info" role="button"><i class="feather icon-edit"></i></a>
                        <a @click.prevent="remove(invoice_type)" href="" class="text-warning" role="button"><i class="feather icon-trash"></i></a>
                      </td>
                    </tr>
                  </tbody>
                </table>
...
<script>
    ...
    export default {
        ...
        methods: {
          ...
          remove: async function (invoice_type) {
            if (await this.$confirm()) {
              this.$inertia.delete(this.route('invoice-types.destroy', invoice_type.id));
              this.$toast(`${invoice_type.name } deleted successfully`);
            }
          }
        }
    }
